...
var Terminal = Terminal || function(containerId) {
  ...
  var worker_ = new Worker('assets/worker.js');
      ...
      switch (cmd) {
        ...
        case 'who':
          output(document.title +
                 ' by Quyen Dam - A web developer<br />&lt;quyendam.2612@gmail.com&gt;');
          break;
        case 'when':
          output('June 1, 2015.');
          break;
        case 'what':
          output('Personal blog.');
          break;
        case 'status':
          output('Coming soon.');
          break;
  ...
  function formatColumns_(entries) {
    ...
    var height = entries.length == 1 ? 'height: ' + (entries.length * 30) + 'px;' :
                 entries.length <= 3 ? 'height: ' + (entries.length * 16) + 'px;' : '';
